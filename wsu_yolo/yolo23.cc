...
#include "../header/gen_ukr_bfxycwh_f16layout_1_28269_17_17_1024_1_1.h"
...
//    long long *Astride = pAstride + tid * 6;
    ...
    //   }
    // }
        for(int fpck = tid * 16; fpck < 28256; fpck+= 8*16){
        ...
        for (int idc = tid; idc<c; idc+=8) {
            //f = 28256~28269
            int idh = 0;
            int idw = 0;
            for(int idf = 28256; idf < 28269; idf++)
            {;
                B[(idf-28256) + idc * w * h * 13 + idw * h * 13 + idh * 13 + 28256*c*w*h]=
                oB[idf * c * w * h + idc * w * h + idw * h + idh];
//                cout<<"offoB "<<idf * c * w * h + idc * w * h + idw * h + idh <<" to offB "<<(idf-28256) + idc * w * h * 13 + idw * h * 13 + idh * 13 + 28256*c*w*h<<endl;
            }
        }


        /*
          oB(28256, c,0,0) -> B(28256*Nc +  0 + c*13)
         */
...
//                        for (int f3 = f4; f3 < (f4 + Tf3 > Nf ? Nf : f4 + Tf3);f3 += Tf2) {
                        ...
                        for (int c3 = c4; c3 < (c4 + Tc3 > Nc ? Nc : c4 + Tc3); c3 += Tc2) {
                            ...
                            for (int xy3 = xy4; xy3 < (xy4 + Txy3 > Nxy ? Nxy : xy4 + Txy3);xy3 += Txy2) {
                                for(int fp = f4+ (tid)*Tfp; fp < std::min(f4+Tf3, Nf); fp += 8*Tfp)
...
//                                  Astride);
                                 ...
                                 partial_ukr_peelker_1_16_unroll1x1(A + Aoffset, B + Boffset, C + Coffset,
                                           ctile, 16, x * y - xy1, 1, 1, Astride, Aoffset,
                                           Boffset, Coffset, b,f,x,y,c,w,h);
                            }//end else
                            }
                            else{
                                //f1 >= 28256
                                Boffset = f1*c*w*h + c2*w*h*13;
                                if (y - y1 > 6) {
                                    partial_ukr_peelker_13(A + Aoffset, B + Boffset, C + Coffset,
                                           ctile, 13, 6, 1, 1, Astride, Aoffset,
                                           Boffset, Coffset, b,f,x,y,c,w,h);
                                }
                                else if (x * y - xy1 >= 6) {
                                    ...
                                    partial_ukr_peelker_13(A + Aoffset, B + Boffset, C + Coffset,
                                           ctile, 13, x * y - xy1, 1, 1, Astride, Aoffset,
                                           Boffset, Coffset, b,f,x,y,c,w,h);
                                }//end else
                            }
