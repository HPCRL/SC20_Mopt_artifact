...
#include "../header/gen_ukr_bfxycwh_f16layout_1_512_34_34_256_3_3.h"
...
//                 xyp < std::min(xy4 + (tid + 1) * Txyp, Nxy); xyp += Txyp)
                ...
                //               xy3 += Txy2) {
                        ...
                        int fp = (tid&3)*Tfp;
                        int xyp = (tid>>2)*Txyp;
//              for(int fp = f4+ (tid&3)*Tfp; fp < std::min(f4+Tf3, Nf); fp += 4*Tfp)
...
//                            Boffset = f1 + c2 * w * h * f;
                            ...
                            Coffset = f1 * x * y + xy1;
                            float* Cblk_addr = C+Coffset;
                            
                            __builtin_prefetch((const void*)(Cblk_addr),1,3);
                            __builtin_prefetch((const void*)(Cblk_addr+x*y),1,3);
                            __builtin_prefetch((const void*)(Cblk_addr+2*x*y),1,3);
                            __builtin_prefetch((const void*)(Cblk_addr+3*x*y),1,3);
                            __builtin_prefetch((const void*)(Cblk_addr+4*x*y),1,3);
                            __builtin_prefetch((const void*)(Cblk_addr+5*x*y),1,3);
                            __builtin_prefetch((const void*)(Cblk_addr+6*x*y),1,3);
                            __builtin_prefetch((const void*)(Cblk_addr+7*x*y),1,3);
                            __builtin_prefetch((const void*)(Cblk_addr+8*x*y),1,3);
                            __builtin_prefetch((const void*)(Cblk_addr+9*x*y),1,3);
                            __builtin_prefetch((const void*)(Cblk_addr+10*x*y),1,3);
                            __builtin_prefetch((const void*)(Cblk_addr+11*x*y),1,3);
                            __builtin_prefetch((const void*)(Cblk_addr+12*x*y),1,3);
                            __builtin_prefetch((const void*)(Cblk_addr+13*x*y),1,3);
                            __builtin_prefetch((const void*)(Cblk_addr+14*x*y),1,3);
                            __builtin_prefetch((const void*)(Cblk_addr+15*x*y),1,3);
                            // begin ukr
                            if (y - y1 > 6) {
                              //                            cout<<"ukr1\n";
                              //                            simu_ukr(A+Aoffset,
