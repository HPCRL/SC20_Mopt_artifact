...
#include "../header/gen_ukr_bfxycwh_f16layout_1_16_48_480_1_3_3.h"
...
#pragma omp parallel  num_threads(8)
  ...
  const int Txy3 = 48*480;
  const int Tf3 = 16;
  const int Tc3 = 1;

  const int Txyp = 48*60;
  const int Tfp = 1;

  const int Txy2 = 48*60;
  const int Tf2 =16;
  const int Tc2 = 1;
...
//              for (int f3 = fp; f3 < (fp + Tfp > Nf ? Nf : fp + Tfp);f3 += Tf2) {
		  for (int xyp = xy4 + tid * Txyp; xyp <  std::min(xy4+Txy3,Nxy); xyp += 8*Txyp)
...
//                            Boffset = f1 + c2 * w * h * f;
                            ...
                            if (y - y1 > 6) {
                              ...
                              for (int i = y - y1; i < 6; i++) {
                                ...
                                continue; // no partial tile for this problem
