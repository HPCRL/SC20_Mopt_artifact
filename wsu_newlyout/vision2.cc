...
//#include "../header/gen_ukr_bfxycwh_1_256_56_56_128_3_3.h"
#include "../header/transpose.h"
#include "../header/gen_ukr_bfxycwh_f16layout_1_256_56_56_128_3_3.h"
...
#pragma omp barrier
    ...
    for (int c4 = 0; c4 < Nc; c4 += Tc3) {
	    ...
	    //              int xyp = xy4 + (tid&3) * Txyp;
//              int fp = f4 + (tid&3) * Tfp;
	    for(int xyp = xy4 + (tid&3)*Txyp; xyp < std::min(Nxy, xy4+Txy3); xyp+= Txyp*4)
		  for(int fp = f4 + (tid>>2) * Tfp; fp < std::min(f4+Tf3, Nf); fp += Tfp *2)
